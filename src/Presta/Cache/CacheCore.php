...
abstract class CacheCore {
    ...
    public function setQuery($query, $result) {
        ...
        if (is_null($this->sql_tables_cached)) {
            $this->sql_tables_cached = $this->get(\Presta\Tools::encryptIV(self::SQL_TABLES_NAME));
            if (!is_array($this->sql_tables_cached))
                $this->sql_tables_cached = array();
        }

        // Store query results in cache
        $key = \Presta\Tools::encryptIV($query);
        ...
        $this->set(\Presta\Tools::encryptIV(self::SQL_TABLES_NAME), $this->sql_tables_cached);
    }
